...
## Repository analysis (V5)

`POST /api/v1/repositories/analyze` — validates URL, ensures clone under `REPOS_CLONE_DIR`, returns `statistics`, `fileTree`, and `dependencyGraph` (JS/TS relative imports only, node/edge capped) with `graphMeta`. Skips `node_modules`, `.git`, `dist`, `build`, `coverage`. Requires **Git** on PATH.
